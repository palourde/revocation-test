...
func revCheck(cert *x509.Certificate) (revoked, ok bool, err error) {
	for _, url := range cert.CRLDistributionPoints {
		...
		fmt.Printf("Checking certificate against CRL at URL %q\n", url)
	...
	fmt.Println("Checking certificate against OCSP")
...
func fetchRemote(url string) (*x509.Certificate, error) {
	fmt.Printf("Fetching remote URL %q\n", url)
	resp, err := http.Get(url)
	if err != nil {
		return nil, err
	}
	...
	p, _ := pem.Decode(in)
	if p != nil {
		fmt.Println("Found PEM data")
		return helpers.ParseCertificatePEM(in)
	}

	fmt.Println("Attempting to parse ASN.1 DER data")
...
func certIsRevokedOCSP(leaf *x509.Certificate, strict bool) (revoked, ok bool, e error) {
	...
	if issuer == nil {
		fmt.Println("OCSP validation failed, found no issuer")
